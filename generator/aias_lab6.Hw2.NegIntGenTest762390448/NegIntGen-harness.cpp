...
#endif
#include <iostream>
class NegIntGen_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    NegIntGen_api_t(VNegIntGen* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_key_in)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_value_bits)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_value_valid)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["NegIntGen.reset"] = 0;
    }
#if VM_TRACE
...
static NegIntGen_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VNegIntGen* top = new VNegIntGen;
    std::string vcdfile = "./generator/aias_lab6.Hw2.NegIntGenTest762390448/NegIntGen.vcd";
